# AUTOGENERATED! DO NOT EDIT! File to edit: Scratch/Notebooks/3.PrepMLData.ipynb (unless otherwise specified).
...
class PrepML:
    ...
    def __init__(self, data_root, interpolate=1, overwrite_cache=False, date_start='2015-11-01', date_end='2020-04-30', date_train_test_cutoff='2019-11-01'):
        """
        ...
        self.nc_path = data_root + '/3.GFSFiltered'+ str(interpolation) + 'xInterpolation/'
        self.processed_path = data_root + '/4.GFSFiltered'+ str(interpolation) + 'xInterpolationZarr/'
        ...
        for region in self.regions.keys():
            for r in self.regions[region]:
                ...
                region_data[r] = xr.open_dataset(nc_path + '15-16/' + '/Region_' + r + '_20160101.nc')

        #Read in all the label data
        self.labels = pd.read_csv(path_to_labels, low_memory=False,
        ...
        self.labels[region_col] = self.labels.apply(lambda x : lookup_forecast_region(x[region_col]), axis=1)
        self.labels = self.labels[self.labels[region_col]!='Unknown region']

        #ensure we are only using label data for regions we are looking at
        self.labels = self.labels[self.labels[region_col].isin(region_zones)]

        #add a season column
        tmp = pd.DataFrame.from_records(self.labels[parsed_date_col].apply(date_to_season))
        ...
        self.labels = self.labels[~self.labels.index.isin(self.labels[(self.labels['season']=='15-16') & (self.labels[region_col]=='Steamboat Zone')].index)]
        self.labels = self.labels[~self.labels.index.isin(self.labels[(self.labels['season']=='16-17') & (self.labels[region_col]=='Front Range Zone')].index)]
        ...
        for r in self.labels[region_col].unique():
        ...
        self.lat_lon_union = pd.DataFrame()
        lat_lon_path = processed_path + 'lat_lon_union.csv'
        if overwrite_cache or not os.path.exists(lat_lon_path):
            ...
            for r in region_data.keys():
                ...
                #cache the data
                self.lat_lon_union.to_csv()
        else:
            #load the cached data
            self.lat_lon_union = pd.read_csv(processed_path + 'lat_lon_union.csv',float_precision='round_trip')
        #join in with the labels so we have a label per lat/lon pair
        self.lat_lon_union = lat_lon_union.set_index(region_col, drop=False).join(self.labels.set_index(region_col, drop=False), how='left', lsuffix='left', rsuffix='right')

        #define the split between train and test
        date_min = np.datetime64(date_start)
        date_max = np.datetime64(date_end)
        train_date_cutoff = np.datetime64(date_train_test_cutoff)
    ...
    def cache_batches(labels, batch_size=64, total_rows=6400, train_or_test='train', lookback_days=14, n_jobs=14):
        """
    ...
    def cache_batches_np(labels, batch_size=50,
                         total_rows=10000,
                         train_or_test='train',
                         lookback_days=180,
    ...
    def create_memmapped(remaining_labels, train_or_test = 'train', num_variables=1131, num_rows = 10000, lookback_days=180, batch=0, batch_size=500):
        """
    ...
    def generate_train_test_local(remaining_labels_train, remaining_labels_test, num_train_files=1, num_test_files=1, num_train_rows_per_file=10000, num_test_rows_per_file=500, num_variables=1131):
        """
        ...
        I have a max of ~50000 rows per batch with 48 gb of ram.
        """

        filenames = []
        for i in range(0, num_train_files):
            remaining_labels_train = create_memmapped(remaining_labels_train, train_or_test = 'train', num_variables=num_variables, num_rows=num_train_rows_per_file, batch=i)
        ...
        for i in range(0, num_test_files):
            remaining_labels_test = create_memmapped(remaining_labels_test, train_or_test = 'test', num_variables=num_variables, num_rows=num_test_rows_per_file, batch=i)
            filenames.append(ml_path + '/Xtest_batch_' + str(i) + '_on_disk.npy')
            #with open(ml_path + 'remaining_labels_test.p', 'wb' ) as file:
            #    pickle.dump(remaining_labels_test, file)

        concat_memapped(filenames, num_variables)
