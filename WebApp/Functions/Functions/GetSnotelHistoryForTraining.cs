...
{
    public static class GetSnotelHistoryForTraining 
    {
        [FunctionName("GetSnotelHistoryForTraining")]
        [return: Queue("filereadytodownloadqueue")]
        public static void Run([TimerTrigger("1 * * * * *", RunOnStartup = true)]TimerInfo myTimer, 
                               ...
                               TraceWriter log)
        {
            ...
            var beginDate = new DateTime(2013, 11, 1, 0, 0, 0, DateTimeKind.Utc);
            var endDate = new DateTime(2013, 11, 30, 23, 0, 0, DateTimeKind.Utc);
            ...
            TableQuery<FileProcessedTracker> dateQuery = new TableQuery<FileProcessedTracker>().Where(
                TableQuery.CombineFilters(
                    ...
                    TableOperators.And,
                    TableQuery.CombineFilters(
                        TableQuery.GenerateFilterConditionForDate("ForecastDate", QueryComparisons.GreaterThanOrEqual, beginDate),
                        TableOperators.And,
                        TableQuery.GenerateFilterConditionForDate("ForecastDate", QueryComparisons.LessThanOrEqual, endDate)
                    )
                )
            );
